...
FalsePositive = 0;
FalseNegative = 0;
up_label=max(test_data_label);
down_label = min(test_data_label);
for k=1:l
    if test_data_label(k)==up_label & predict_label(k)==up_label  %真阳性
        TruePositive = TruePositive +1;
    end
    if test_data_label(k)==down_label & predict_label(k)==down_label %真阴性
        TrueNegative = TrueNegative +1;
    end 
    if test_data_label(k)==down_label & predict_label(k)==up_label  %假阳性
        FalsePositive = FalsePositive +1;
    end

    if test_data_label(k)==up_label & predict_label(k)==down_label  %假阴性
